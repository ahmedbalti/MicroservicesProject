...
import com.esprit.tn.candidacy_microservice.Entities.Candidacy.*;
import com.esprit.tn.candidacy_microservice.Repository.Candidacy.ICandidacyRepository;
import com.esprit.tn.candidacy_microservice.Repository.Candidacy.IResultRepository;
...
public class CandidacyServiceImpl implements ICandidacyService {
    ...
    public Candidacy updateCandidacyStatus(Integer idCandidacy) {
        ...
        if (candidacy.getScoree() > 20 && candidacy.getMoyenneGenerale() > 16) {
    ...
    public Candidacy updateCandidatureStatus(Integer idCandidacy) {
        Candidacy candidature = candidacyRepository.findById(idCandidacy)
                .orElseThrow(() -> new RuntimeException("Candidacy not found with id " + idCandidacy));
       // if (candidature.getStatusCandidacy() == StatusCandidacy.EN_ATTENTE && candidature.getDisponibilite() == Disponibilite.IMMEDIATE && candidature.getProfil() == Profil.ETUDIANT) {
        if (candidature.getStatusCandidacy() == StatusCandidacy.EN_ATTENTE && candidature.getDisponibilite() == Disponibilite.IMMEDIATE ) {
            candidature.setStatusCandidacy(StatusCandidacy.ACCEPTEE);
        } else {
    ...
    public void accepterOuRefuserCandidature(Integer idCandidacy) {
        ...
        //if (candidature.getProfil() == Profil.ENSEIGNANT && candidature.getAnneeExperience() > 2 && candidature.getStatusCandidacy() == StatusCandidacy.EN_ATTENTE  && candidature.getDisponibilite() == Disponibilite.IMMEDIATE) {
        if (candidature.getAnneeExperience() > 2 && candidature.getStatusCandidacy() == StatusCandidacy.EN_ATTENTE && candidature.getDisponibilite() == Disponibilite.IMMEDIATE) {
            candidature.setStatusCandidacy(StatusCandidacy.ACCEPTEE);
        } else {
            candidature.setStatusCandidacy(StatusCandidacy.REFUSEE);
        }
    ...
    public Optional<Candidacy> findById(Integer id) {
        return candidacyRepository.findById(id);
    }

    public Candidacy save(Candidacy employee) {
